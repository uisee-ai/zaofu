...
4. Check portability criteria.
   ...
   - Needs adaptation when event names, dirty-file policy, provider backend,
     ...
     `dispatch_routing_queries.py` `_requires_task_ref_for_progress_event`、`terminal_ledger.py` `TERMINAL_SUCCESS_EVENTS`、`topology.py`);
...
- **verify/judge reader stage 的 `target_ref`**(bizsim r4 F9,launch-blocking):
  ...
  `fanout.child.workdir_mismatch`(`orchestrator_fanout.py` `_pin_reader_target_or_reject`);且知晓
  pin-commit/delta 门语义——同一 `target_commit` 无新 commit 的重审会被
  `fanout.retrigger.suppressed`(`orchestrator_fanout.py` `_delta_gate_allows`)抑制,避免
  "judge 审基线树"级别的静默失败。
...
- `zf recover workflow` or equivalent dry-run can report pending resume
  ...
  (`run_manager.py` `_TERMINAL_SIGNAL_EVENTS`/`_POST_TERMINAL_WORK_EVENTS`、
  `quiescent.py` `QUIESCENT_ENTERED_EVENT`/`QUIESCENT_EXITED_EVENT`);任务级续跑走 attempt spine
  ...
  `lane_micro_loop.py` `CONTINUATION_EVENT`、`candidate_rework.py` `plan_candidate_rework`)。审 unattended 长程续跑
  能力时按这两条 spine 核对。
