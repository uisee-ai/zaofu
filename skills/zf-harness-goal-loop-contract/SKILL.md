...
（store.py:122 只认 `_VALID_STATUSES`，且无 delivered）。
...
`goal.enabled`（schema.py:1242，灰度默认关）开启后，goal 回路的生命周期由这组
**kernel 事件**驱动（均登记于 known_types.py:388-390）。投影
`build_run_goal_projection` 消费它们给出 `status`（run_manager.py:1620-1676）。
技能判定 goal 状态时读这些事件 / `zf goal show`，**不要靠 LLM 叙述**：

| 事件 | 发射点（anchor） | 语义 / 投影 status |
|---|---|---|
| `run.goal.started` | `zf submit` accepted 后 kernel 铸造（cli/flow.py:1106-1116，gated by `goal.enabled`） | goal 开跑，`run_id`=correlation_id；投影 → `active`（run_manager.py:1628） |
| `run.goal.updated` | `zf goal set`（actor=operator，cli/goal.py:105）；cost/usage/provider-stop 限流时 kernel 补发（orchestrator_reactor.py:4672/4939，status=`budget_limited`/`usage_limited` 等） | 改 objective/status，投影按 payload.status 更新（run_manager.py:1633）；**也是唤醒事件**（见下） |
| `run.goal.completed` | kernel 在真终态成功（`judge.passed`）落地且仍有活 goal 时铸造（`run_goal_completion_event`，run_manager.py:1679-1707） | 投影 → `complete`（run_manager.py:1638），列入终态信号 `_TERMINAL_SIGNAL_EVENTS`（run_manager.py:112） |
| `run.goal.blocked` | goal 阻塞态；投影 → `blocked`（run_manager.py:1642），登记为问题（event_problem_registry.py:558）/失败信号（failure_signals.py:139）。`zf goal set --status blocked` 走 `run.goal.updated` 同样落 `blocked` | 走 escalate/人工，不再自动重试 |
| `run.goal.quiescent.entered` | `mark_quiescent_transition`（quiescent.py:132），tick 服务每 tick 判（tick_services.py:139） | 终局 escalate 后进入静默：tick 服务全体跳过（`return TickServiceResult()`），不再空烧 |
| `run.goal.quiescent.exited` | 同上（quiescent.py:143） | 唤醒后退出静默，tick 恢复点火 |
...
（run_id 非空）**时确定性铸造（run_manager.py:1682-1695）。这正是"provider goal
...
- **退出 / 唤醒事件**（quiescent.py:28-36）：`user.message` / `user.intent.submitted` /
