...
class TaskRefManager:
    ...
    def _reject_out_of_scope_task_ref(
        ...
        *,
        ...
        if not changed:
            changed = self._changed_files_for_source_commit(source_commit, payload)
        if not changed:
    ...
    def _changed_files_for_source_commit(
        ...
        if not base:
            return []
        try:
            merge_base = self._git("merge-base", source_commit, base).strip()
            raw = self._git("diff", "--name-only", f"{merge_base}..{source_commit}")
        except Exception:
            return []
        return [
...
def _runtime_materialized_dirty_files(paths: list[str]) -> list[str]:
    return [
        path for path in paths
        if path.startswith(_RUNTIME_MATERIALIZED_DIRTY_PREFIXES)
    ]
