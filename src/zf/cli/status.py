...
def run(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "dispatch", False):
        return _print_dispatch_diagnostics(ctx)

    if getattr(args, "workers", False):
        return _print_workers(state_dir, event_log, ctx.config_path)
...
def _print_dispatch_diagnostics(ctx) -> int:
...
def _print_workers(state_dir: Path, event_log: EventLog, cfg_path: Path) -> int:
