...
// --- partitionToolRun (streaming run keeps tail of 3) ---
const sixTools = Array.from({ length: 6 }, (_, i) => part(`tool${i}`, "tool"));
const live = partitionToolRun(sixTools, true);
assert(live.standalone.length === 3, `live run exposes 3, got ${live.standalone.length}`);
assert(live.grouped.length === 3, `live run folds 3, got ${live.grouped.length}`);
...
assert(seg.standalone.length === 3, "streaming tail of 3 exposed");
